...
/*   Updated: 2022/01/13 12:48:56 by jmaia            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		free_stacks_without_content(void *stacks);
...
static t_sort	*init_sorter(t_stacks *stacks)
{
	...
	if (stack_a_the_list)
	{
		...
		sorter->the_list = ft_lstnew(get_stacks(stack_a_the_list, 0));
	}
	sorter->paths = 0;
...
void	free_sorter(t_sort **sorter)
{
	...
	ft_lstclear(&(*sorter)->the_list, &free_stacks_without_content);
